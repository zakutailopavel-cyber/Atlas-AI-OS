...
function AvatarStudio({models,items,assets,close,savePortrait,saveAsset,attach}:{models:Model[];items:Item[];assets:Asset[];close:()=>void;savePortrait:(model:Model,url:string)=>Promise<void>;saveAsset:(model:Model,url:string,jobId:string)=>Promise<void>;attach:(itemId:string,url:string)=>Promise<void>}){
  const [modelId,setModelId]=useState(models[0]?.id||""),[kind,setKind]=useState<"avatar"|"scene">("avatar"),[prompt,setPrompt]=useState(""),[style,setStyle]=useState("Фотореалистичный lifestyle"),[jobs,setJobs]=useState<AvatarJob[]>([]),[targetId,setTargetId]=useState(""),[refineSource,setRefineSource]=useState(""),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  async function generate(){if(!model||!prompt)return;setBusy(true);setError("");try{const r=await fetch("/api/avatar",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({model_id:model.id,kind,prompt,style,count:kind==="scene"?1:4,source_url:refineSource||null})}),d=await r.json();if(!r.ok)throw new Error(d.error||"Не удалось создать задание");setRefineSource("");setPrompt("");await refresh()}catch(e){setError(e instanceof Error?e.message:"Ошибка генерации")}finally{setBusy(false)}}
