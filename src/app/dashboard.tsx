...
function AvatarStudio({models,items,assets,close,savePortrait,saveAsset,attach}:{models:Model[];items:Item[];assets:Asset[];close:()=>void;savePortrait:(model:Model,url:string)=>Promise<void>;saveAsset:(model:Model,url:string,jobId:string)=>Promise<void>;attach:(itemId:string,url:string)=>Promise<void>}){
  const [modelId,setModelId]=useState(models[0]?.id||""),[kind,setKind]=useState<"avatar"|"scene">("avatar"),[prompt,setPrompt]=useState(""),[style,setStyle]=useState("Фотореалистичный lifestyle"),[jobs,setJobs]=useState<AvatarJob[]>([]),[targetId,setTargetId]=useState(""),[refineSource,setRefineSource]=useState(""),[busy,setBusy]=useState(false),[retrying,setRetrying]=useState(""),[error,setError]=useState("");
