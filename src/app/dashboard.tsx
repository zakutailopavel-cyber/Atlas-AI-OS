...
type AvatarJob = { id:string; model_id:string; kind:"avatar"|"scene"|"faceswap"; prompt:string; style:string; status:string; output_urls:string[] | null; error:string | null; created_at:string };
...
function AvatarStudio({models,items,assets,close,savePortrait,saveAsset,attach,initialModelId}:{models:Model[];items:Item[];assets:Asset[];close:()=>void;savePortrait:(model:Model,url:string)=>Promise<void>;saveAsset:(model:Model,url:string,jobId:string)=>Promise<void>;attach:(itemId:string,url:string)=>Promise<void>;initialModelId?:string}){
  const [modelId,setModelId]=useState(initialModelId||models[0]?.id||""),[kind,setKind]=useState<"avatar"|"scene"|"faceswap">("avatar"),[prompt,setPrompt]=useState(""),[style,setStyle]=useState("Фотореалистичный lifestyle"),[framing,setFraming]=useState("waist_up"),[jobs,setJobs]=useState<AvatarJob[]>([]),[targetId,setTargetId]=useState(""),[refineSource,setRefineSource]=useState(""),[previewUrl,setPreviewUrl]=useState(""),[busy,setBusy]=useState(false),[retrying,setRetrying]=useState(""),[error,setError]=useState(""),[uploadedPhotoUrl,setUploadedPhotoUrl]=useState(""),[uploading,setUploading]=useState(false);
  const model=models.find(m=>m.id===modelId);
  const client=useMemo(()=>createClient(),[]);
  async function uploadPhoto(file:File){
    if(!model)return;setUploading(true);setError("");
    try{
      const path=`faceswap-uploads/${model.id}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"_")}`;
      const {error:uploadError}=await client.storage.from("atlas-assets").upload(path,file,{contentType:file.type||"image/jpeg"});
      if(uploadError)throw uploadError;
      const {data}=client.storage.from("atlas-assets").getPublicUrl(path);
      setUploadedPhotoUrl(data.publicUrl);
    }catch(e){setError(e instanceof Error?e.message:"Не удалось загрузить фото")}finally{setUploading(false)}
  }
  ...
  async function generate(count=1){if(!model||(kind==="scene"&&!prompt.trim())||(kind==="faceswap"&&!uploadedPhotoUrl))return;setBusy(true);setError("");try{const r=await fetch("/api/avatar",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({model_id:model.id,kind,prompt,style,framing,count:kind==="scene"||kind==="faceswap"?1:count,source_url:refineSource||null,base_photo_url:kind==="faceswap"?uploadedPhotoUrl:null})}),d=await r.json();if(!r.ok)throw new Error(d.error||"Не удалось создать задание");setRefineSource("");if(kind==="scene")setPrompt("");if(kind==="faceswap"){setPrompt("");setUploadedPhotoUrl("")}await refresh()}catch(e){setError(e instanceof Error?e.message:"Ошибка генерации")}finally{setBusy(false)}}
