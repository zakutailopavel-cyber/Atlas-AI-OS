...
const nav:Array<[View,string,string]>=[["overview","Обзор","⌂"],["model","Модель","◉"],["create","Создать задачу","＋"],["queue","Очередь","≡"],["moderation","Модерация","✓"]];
...
function passportText(model:Model,key:string,fallback:string){const value=model.visual_passport?.[key];return Array.isArray(value)?value.join(", "):String(value||fallback)}
function passportBoolean(model:Model,key:string){return Boolean(model.visual_passport?.[key])}
...
export default function StudioV2Client({models:initialModels,references,items,accounts,initialTasks}:{models:Model[];references:Reference[];items:Item[];accounts:Account[];initialTasks:Task[]}){
  ...
  async function saveAutomation(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      const windows=String(form.get("posting_windows")||"").split(",").map(value=>value.trim()).filter(Boolean);
      const response=await fetch("/api/studio/models",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({
        id:model.id,
        timezone:String(form.get("timezone")||""),
        market:String(form.get("market")||""),
        language:String(form.get("language")||""),
        posting_windows:windows,
        posts_per_day:Number(form.get("posts_per_day")||1),
        automation_enabled:form.get("automation_enabled")==="on",
        last_posted_at:model.visual_passport?.last_posted_at||null,
      })});
      const data=await response.json();
      if(!response.ok)throw new Error(data.error||"Не удалось сохранить расписание");
      setModels(current=>current.map(entry=>entry.id===model.id?data.model:entry));
      setMessage("Настройки автоматизации сохранены. n8n будет учитывать локальное время модели.");
    }catch(value){setError(value instanceof Error?value.message:"Ошибка сохранения")}finally{setBusy(false)}
  }
  ...
  async function moderate(action:"approve"|"request_changes"){
    ...
    try{
      ...
      await refreshTasks();setNotes("");setMessage(action==="approve"?"Результат одобрен и передан в контент-поток.":"Задача возвращена на переделку и событие отправлено в n8n.");
    }catch(value){setError(value instanceof Error?value.message:"Ошибка")}finally{setBusy(false)}
  }
  ...
  return <main className="ops-shell">
    <aside className="ops-sidebar"><Link href="/" className="ops-logo"><span>◈</span>Atlas</Link><nav>{nav.map(([id,label,icon])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}><span>{icon}</span>{label}{id==="moderation"&&review>0?<em>{review}</em>:null}</button>)}</nav><div className="ops-sidebar-foot"><span className="ops-user">P</span><div><b>Павел</b><small>Администратор</small></div></div></aside>
    <section className="ops-main">
      <header className="ops-topbar"><div><small>ATLAS CONTENT OPERATIONS</small><h1>{view==="overview"?"Обзор":view==="model"?"Профиль модели":view==="create"?"Создание задачи":view==="queue"?"Очередь и генерация":"Модерация"}</h1></div><select value={model.id} onChange={event=>{setModelId(event.target.value);setSelectedRefs([])}}>{models.map(entry=><option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></header>
      {message&&<div className="ops-alert success">{message}</div>}{error&&<div className="ops-alert error">{error}</div>}

      {view==="overview"&&<><section className="ops-kpis"><article><small>Активные модели</small><strong>{models.filter(entry=>entry.status==="active").length}</strong><span>Всего {models.length}</span></article><article><small>В очереди</small><strong>{pending}</strong><span>{claimed} в работе</span></article><article><small>Ожидают модерации</small><strong>{review}</strong><span>Требуют решения</span></article><article><small>Запланировано</small><strong>{scheduled}</strong><span>Ближайшие публикации</span></article></section><div className="ops-grid two"><section className="ops-card"><header><div><small>АВТОМАТИЗАЦИИ</small><h2>Что происходит сейчас</h2></div><button onClick={()=>setView("queue")}>Открыть очередь</button></header><div className="ops-timeline"><article><span className="green"/><div><b>n8n проверяет due-персон</b><small>Каждый час по локальному времени модели</small></div><em>Активно</em></article><article><span className="blue"/><div><b>GPT формирует брифы</b><small>Тренды, референсы, caption angle</small></div><em>{pending} задач</em></article><article><span className="orange"/><div><b>Модерация результатов</b><small>Approve или Request changes</small></div><em>{review} ожидают</em></article></div></section><section className="ops-card"><header><div><small>БЛИЖАЙШИЙ КОНТЕНТ</small><h2>Публикации</h2></div></header><div className="ops-content-list">{items.slice(0,6).map(item=><article key={item.id}><span style={item.asset_url?{backgroundImage:`url(${item.asset_url})`}:undefined}/><div><b>{item.title}</b><small>{item.platform||"Платформа не выбрана"} · {dateLabel(item.publish_at)}</small></div><em>{item.status}</em></article>)}{!items.length&&<p>Контент пока не создан.</p>}</div></section></div></>}

      {view==="model"&&<><section className="ops-identity-card"><div className="ops-face" style={avatar(model)?{backgroundImage:`url(${avatar(model)})`}:undefined}>{!avatar(model)&&model.name.slice(0,1)}<span>🔒 Главный face reference</span></div><div className="ops-identity-copy"><div className="ops-title-row"><div><small>AI-МОДЕЛЬ</small><h2>{model.name}</h2><p>{model.handle||"Handle не указан"}</p></div><span className="ops-lock">🔒 Идентичность зафиксирована</span></div><p>{model.bio||"Описание модели ещё не заполнено."}</p><div className="ops-meta"><div><small>Ниша</small><b>{model.niche||"Не указана"}</b></div><div><small>Часовой пояс</small><b>{passportText(model,"timezone","Europe/Tallinn")}</b></div><div><small>Рынок</small><b>{passportText(model,"market","Не задан")}</b></div><div><small>Язык</small><b>{passportText(model,"language","Русский")}</b></div><div><small>Окна публикаций</small><b>{passportText(model,"posting_windows","Не настроены")}</b></div><div><small>Площадки</small><b>{modelAccounts.map(account=>account.platform).join(", ")||"Не подключены"}</b></div><div><small>Материалы</small><b>{modelItems.length}</b></div><div><small>Статус</small><b>{model.status}</b></div></div></div></section>
        <form key={model.id} className="ops-card form ops-automation-form" onSubmit={saveAutomation}><header><div><small>N8N SCHEDULE</small><h2>Автоматизация публикаций</h2></div><label className="ops-switch"><input name="automation_enabled" type="checkbox" defaultChecked={passportBoolean(model,"automation_enabled")}/><span/>Включена</label></header><div className="ops-automation-fields"><label>Часовой пояс<input name="timezone" defaultValue={passportText(model,"timezone","Europe/Tallinn")} placeholder="America/Los_Angeles"/></label><label>Рынок<input name="market" defaultValue={passportText(model,"market","")} placeholder="США, Калифорния"/></label><label>Язык<input name="language" defaultValue={passportText(model,"language","Русский")} placeholder="English"/></label><label>Постов в день<input name="posts_per_day" type="number" min="1" max="6" defaultValue={Number(model.visual_passport?.posts_per_day||1)}/></label><label className="wide">Окна публикаций<input name="posting_windows" defaultValue={passportText(model,"posting_windows","09:00, 19:00")} placeholder="09:00, 19:00"/><small>Укажи время через запятую в локальном часовом поясе модели.</small></label></div><footer><div><b>Последняя публикация</b><small>{dateLabel(String(model.visual_passport?.last_posted_at||"")||null)}</small></div><button className="ops-primary" disabled={busy}>{busy?"Сохраняем…":"Сохранить расписание"}</button></footer></form>
