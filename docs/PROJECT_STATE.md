...
| 03 — Контент-фабрика | Публикации, тексты, изображения, материалы, календарь | Content Pipeline v1 (approval gate) в `main`; PR #106 сделал AI-disclosure обязательным в `/api/approve` (HTTP 400 при пустом поле — раньше был только мягкий UI-варнинг); trend-aware генерация недели (`/api/trends` → валидация по forbidden_topics/disclosure → существующий `/api/plan-week`), `social_accounts` как источник списка площадок, статические инструкции по загрузке и `PostTextEditor` вместо простых textarea для caption/prompt/комментария | Собрать метрики использования трендового шага и решить, нужен ли отдельный UI для истории trend-брифов |
...
| 05 — Backend и инфраструктура | Supabase, Storage, RLS, Vercel, Modal, auth, расходы | PR #105 слит: Cost Governor (`cost_ledger`/`budget_limits`, существовавшие ранее только для `/api/fan-reply`) подключён также к `/api/avatar` (Modal) и `/api/generate` (OpenAI) — `is_over_budget` перед платным вызовом, запись в `cost_ledger` после | Подготовить отдельную counts-only read-only preflight specification и изолированный rehearsal без production; production execution и RLS cutover запрещены без backup/PITR gate, `created_by` нельзя автоматически использовать как `owner_id` |
...
- Повтор задания создаёт новую GPU-генерацию; учёт стоимости теперь есть (`cost_ledger`/`budget_limits`, PR #105), но защиты от случайных повторов на UI-уровне ещё нет.
...
4. P1: обязательный AI-disclosure в publish payload реализован (PR #106, `/api/approve`); осталась ручная проверка актуальных правил/KYC выбранной площадки перед запуском.
...
6. P1: Cost Governor подключён к `/api/fan-reply`, `/api/avatar` и `/api/generate` (PR #105); осталась защита от случайных повторов задания на UI-уровне.
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | 03 | Завершено | PR #106 слит: `/api/approve` (единственный путь в status "ready") теперь возвращает HTTP 400, если `body.disclosure` пустой или состоит из пробелов — раньше UI дашборда только мягко предупреждал перед сохранением черновика, жёсткой проверки не было нигде. UI не менялся: `CharacterPage.approve()` уже отправлял `disclosure` в каждом вызове | PR #106 |
| 2026-08-02 | 05 | Завершено | PR #105 слит: Cost Governor (`cost_ledger`/`budget_limits`, существовавшие только для `/api/fan-reply`) подключён к `/api/avatar` (Modal GPU: avatar/scene/faceswap, оценка $0.025/$0.04/$0.06) и `/api/generate` (OpenAI, $0.002/1K токенов) — `is_over_budget` проверяется перед платным вызовом (HTTP 402 при превышении), запись в `cost_ledger` после успешной постановки job/ответа модели | PR #105 |
| 2026-08-02 | 05 | Завершено | PR #100 слит: ручная загрузка фото в Студии персонажа для face-swap (IP-Adapter FaceID-Plus-v2, SDXL inpaint по маске лица поверх реального фото — фон/свет не трогаются). `FaceSwapGenerator.generate()` подключён к очереди `generation_jobs`; additive migration `202608021500` расширила `generation_jobs_kind_check` до `('avatar','scene','faceswap')` (применено к production Supabase до merge, одобрено пользователем); pgTAP обновлён (migration chain + проверка constraint). Новая вкладка «Загрузить фото» в AvatarStudio грузит файл в Supabase Storage и переиспользует существующую логику сохранения/прикрепления к посту. Прототип на тестовом Unsplash-фото проверен вживую в две итерации (маска доработана после первого дефекта — склейка с волосами/лямками); источник фото для продакшна — собственная референс-библиотека пользователя | PR #100 |
