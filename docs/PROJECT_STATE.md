...
| --- | --- | --- | --- | --- |
| 2026-08-02 | 04 | Завершено | PR #109 слит: пользователь указал прямо, что PR #108 был «просто цвет поменял», а не полноценный дизайн — живой обход production (`atlas.epkoolitus.ee`, авторизованная сессия) вскрыл три структурные проблемы, которые чистый recolor не решал: сайдбар использовал Unicode-глиф-заглушки вместо иконок (заменены на 7 inline SVG через `NAV_ICONS` в `dashboard.tsx`), два поля в `.social-preview` (`Ссылка на изображение`, `Текст публикации`) были голыми `<label>` без обёртки `.form`/`.settings` и рендерились нестилизованным текстом (добавлены точечные правила в `production.css`), кнопка «Сохранить профиль» на карточке персонажа была плавающей без фона/рамки (обёрнута в toolbar-карточку в `character.css`). DOM-структура, API, Supabase и Modal не менялись | PR #109 |
| 2026-08-02 | 04 | Завершено | PR #108 слит: полный CSS-token редизайн панели (`globals.css` + 10 файлов стилей) под тёмную премиальную тему — единая система custom properties (`--bg/--surface/--border/--text/--accent/--radius/--shadow` и т.д.), применена ко всем разделам дашборда; область ограничена CSS, JSX/DOM/API/данные не менялись | PR #108 |
